...
Using Adjusted R2 for zero-inflated models for the total Sphagnum model, and Ferrari's R2 for the other models.

```{r}
r2_zeroinflated(mod_abund_tot_Sphagnum_temp_AR)$R2_adjusted
r2_ferrari(mod_abund_Erio_nozi_temp_AR,correct_bounds=T)
r2_ferrari(mod_abund_Erica_nozi_temp_AR,correct_bounds=T)
r2_ferrari(mod_abund_Carex_nozi_temp_AR,correct_bounds=T)
```

### Appendix S4, Table S2.

Using Adjusted R2 for zero-inflated models for all models.

```{r}
r2_zeroinflated(mod_abund_Rubellum_temp)$R2_adjusted
r2_zeroinflated(mod_abund_Balticum_temp)$R2_adjusted
r2_zeroinflated(mod_abund_Cuspidata_temp)$R2_adjusted
```
### Appendix S4, Table S3.
...
```{r}
r2_zeroinflated(mod_abund_Medium_temp)$R2_adjusted
r2_zeroinflated(mod_abund_Fuscum_temp)$R2_adjusted
```
...
```{r}
r2_ferrari(mod_abund_Erica_nozi_temp_AR_sig_ints,correct_bounds=T)
```
...
```{r}
RMSE_tot_Sphagnum<-RMSE(predict(mod_abund_tot_Sphagnum_temp_AR,newdata=dat0,
                                type="response"),dat0$tot_Sphagnum_prop)
RMSE_Erio<-RMSE(predict(mod_abund_Erio_nozi_temp_AR,newdata=dat0,
                        type="response"),dat0$Erio_prop)
RMSE_Erica<-RMSE(predict(mod_abund_Erica_nozi_temp_AR,newdata=dat0_Erica,
                        type="response"),dat0_Erica$Erica_prop)
RMSE_Carex<-RMSE(predict(mod_abund_Carex_nozi_temp_AR,newdata=dat0,
                         type="response"),dat0$Carex_prop)
MAE_tot_Sphagnum<-MAE(predict(mod_abund_tot_Sphagnum_temp_AR,newdata=dat0,
                                type="response"),dat0$tot_Sphagnum_prop)
MAE_Erio<-MAE(predict(mod_abund_Erio_nozi_temp_AR,newdata=dat0,
                        type="response"),dat0$Erio_prop)
MAE_Erica<-MAE(predict(mod_abund_Erica_nozi_temp_AR,newdata=dat0_Erica,
                        type="response"),dat0_Erica$Erica_prop)
MAE_Carex<-MAE(predict(mod_abund_Carex_nozi_temp_AR,newdata=dat0,
                         type="response"),dat0$Carex_prop)
RMSE_tot_Sphagnum
RMSE_Erio
RMSE_Erica
...
```{r}
RMSE_Rubellum<-RMSE(predict(mod_abund_Rubellum_temp,
                            newdata=subset(data_peat,!is.na(Rubellum_prop)&
                                             !is.na(temp)&!is.na(moist)&
                                             !is.na(nutrient)&!is.na(fire)&
                                             !is.na(dry)),
                            type="response"),
                    subset(data_peat,!is.na(Rubellum_prop)&!is.na(temp)&
                             !is.na(moist)&!is.na(nutrient)&!is.na(fire)&
                             !is.na(dry))$Rubellum_prop)
RMSE_Balticum<-RMSE(predict(mod_abund_Balticum_temp,
                            newdata=subset(data_peat,!is.na(Balticum_prop)&
                                             !is.na(temp)&!is.na(moist)&
                                             !is.na(nutrient)&!is.na(fire)&
                                             !is.na(dry)),
                            type="response"),
                    subset(data_peat,!is.na(Balticum_prop)&!is.na(temp)&
                             !is.na(moist)&!is.na(nutrient)&!is.na(fire)&
                             !is.na(dry))$Balticum_prop)
RMSE_Cuspidata<-RMSE(predict(mod_abund_Cuspidata_temp,
                            newdata=subset(data_peat,!is.na(Cuspidata_prop)&
                                             !is.na(temp)&!is.na(moist)&
                                             !is.na(nutrient)&!is.na(fire)&
                                             !is.na(dry)),
                            type="response"),
                    subset(data_peat,!is.na(Cuspidata_prop)&!is.na(temp)&
                             !is.na(moist)&!is.na(nutrient)&!is.na(fire)&
                             !is.na(dry))$Cuspidata_prop)
MAE_Rubellum<-MAE(predict(mod_abund_Rubellum_temp,
                            newdata=subset(data_peat,!is.na(Rubellum_prop)&
                                             !is.na(temp)&!is.na(moist)&
                                             !is.na(nutrient)&!is.na(fire)&
                                             !is.na(dry)),
                    ...
                    subset(data_peat,!is.na(Rubellum_prop)&!is.na(temp)&
                             !is.na(moist)&!is.na(nutrient)&!is.na(fire)&
                             !is.na(dry))$Rubellum_prop)
MAE_Balticum<-MAE(predict(mod_abund_Balticum_temp,
                            newdata=subset(data_peat,!is.na(Balticum_prop)&
                                             !is.na(temp)&!is.na(moist)&
                                             !is.na(nutrient)&!is.na(fire)&
                                             !is.na(dry)),
                    ...
                    subset(data_peat,!is.na(Balticum_prop)&!is.na(temp)&
                             !is.na(moist)&!is.na(nutrient)&!is.na(fire)&
                             !is.na(dry))$Balticum_prop)
MAE_Cuspidata<-MAE(predict(mod_abund_Cuspidata_temp,
                            newdata=subset(data_peat,!is.na(Cuspidata_prop)&
                                             !is.na(temp)&!is.na(moist)&
                                             !is.na(nutrient)&!is.na(fire)&
                                             !is.na(dry)),
...
```{r}
RMSE_Medium<-RMSE(predict(mod_abund_Medium_temp,
                            newdata=subset(data_peat,!is.na(Medium_prop)&
                                             !is.na(temp)&!is.na(moist)&
                                             !is.na(nutrient)&!is.na(fire)&
                                             !is.na(dry)),
                            type="response"),
                    subset(data_peat,!is.na(Medium_prop)&!is.na(temp)&
                             !is.na(moist)&!is.na(nutrient)&!is.na(fire)&
                             !is.na(dry))$Medium_prop)
RMSE_Fuscum<-RMSE(predict(mod_abund_Fuscum_temp,
                            newdata=subset(data_peat,!is.na(Fuscum_prop)&
                                             !is.na(temp)&!is.na(moist)&
                                             !is.na(nutrient)&!is.na(fire)&
                                             !is.na(dry)),
                            type="response"),
                    subset(data_peat,!is.na(Fuscum_prop)&!is.na(temp)&
                             !is.na(moist)&!is.na(nutrient)&!is.na(fire)&
                             !is.na(dry))$Fuscum_prop)
MAE_Medium<-MAE(predict(mod_abund_Medium_temp,
                            newdata=subset(data_peat,!is.na(Medium_prop)&
                                             !is.na(temp)&!is.na(moist)&
                                             !is.na(nutrient)&!is.na(fire)&
                                             !is.na(dry)),
                    ...
                    subset(data_peat,!is.na(Medium_prop)&!is.na(temp)&
                             !is.na(moist)&!is.na(nutrient)&!is.na(fire)&
                             !is.na(dry))$Medium_prop)
MAE_Fuscum<-MAE(predict(mod_abund_Fuscum_temp,
                            newdata=subset(data_peat,!is.na(Fuscum_prop)&
                                             !is.na(temp)&!is.na(moist)&
                                             !is.na(nutrient)&!is.na(fire)&
                                             !is.na(dry)),
...
```{r}
RMSE_Erica_ints<-RMSE(predict(mod_abund_Erica_nozi_temp_AR_sig_ints,
                              newdata=dat0_Erica,
                              type="response"),dat0_Erica$Erica_prop)
MAE_Erica_ints<-MAE(predict(mod_abund_Erica_nozi_temp_AR_sig_ints,
                              newdata=dat0_Erica,
                              type="response"),dat0_Erica$Erica_prop)
RMSE_Erica_ints
MAE_Erica_ints
